...
final class Base extends Prefab implements ArrayAccess
{
	...
	function extract($arr, $prefix)
	{
		$out = [];
		foreach (
			preg_grep('/^' . preg_quote($prefix, '/') . '/', array_keys($arr))
			as $key
		)
					...
					switch ($type) {
						...
						case 'date':
							if ($php81) {
								...
								$dateType = (empty($mod) || $mod == 'short') ? IntlDateFormatter::SHORT : ($mod == 'full' ? IntlDateFormatter::FULL : IntlDateFormatter::LONG);
						...
						case 'time':
							if ($php81) {
								...
								$formatter = new IntlDateFormatter(
									...
									IntlDateFormatter::NONE,
									(empty($mod) || $mod == 'short')
										? IntlDateFormatter::SHORT : ($mod == 'full' ? IntlDateFormatter::LONG : IntlDateFormatter::MEDIUM),
	...
	function error($code, $text = '', ?array $trace = NULL, $level = 0)
	{
		...
		if ((!$handler ||
			...
			if ($this->hive['CLI'])
				...
				echo $this->hive['AJAX'] ?
					json_encode($error) : ('<!DOCTYPE html>' . $eol .
						'<html>' . $eol .
						'<head>' .
	...
	function run()
	{
		...
		foreach ($this->hive['ROUTES'] as $pattern => $routes) {
			...
			if (isset($route[$this->hive['VERB']]) && !$preflight) {
				if (
					$this->hive['REROUTE_TRAILING_SLASH'] === TRUE &&
					$this->hive['VERB'] == 'GET' &&
